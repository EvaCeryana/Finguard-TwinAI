...
export function RiskDashboard() {
  ...
  const priorityControls = assessment.controlPlan.filter((control) => control.priority === "immediate");
  const mainControls = (priorityControls.length > 0 ? priorityControls : assessment.controlPlan).slice(0, 4);
  const hiddenControls = assessment.controlPlan.filter((control) => !mainControls.some((item) => item.id === control.id));
  const visibleControls = showMoreControls ? [...mainControls, ...hiddenControls] : mainControls;
  const visibleStakeholders = showMoreStakeholders ? assessment.affectedStakeholders : assessment.affectedStakeholders.slice(0, 5);
  const hiddenStakeholderCount = Math.max(assessment.affectedStakeholders.length - visibleStakeholders.length, 0);
    ...
    <div className="page-dashboard">
      ...
      <section className="dashboard-section">
        <div className="section-header-row">
          ...
          {hiddenControls.length > 0 && (
            <button className="btn btn--ghost btn--sm" type="button" onClick={() => setShowMoreControls((current) => !current)}>
              {showMoreControls ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
              {showMoreControls ? t("showLess") : t("showMore")}
            </button>
          )}
        </div>
        <ControlPlanTable controls={visibleControls} />
      </section>
      ...
      <section className="dashboard-section">
        ...
        {assessment.affectedStakeholders.length > 5 && (
          <button className="btn btn--ghost btn--sm" type="button" onClick={() => setShowMoreStakeholders((current) => !current)}>
            {showMoreStakeholders ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
            {showMoreStakeholders ? t("showLess") : `${t("showMore")} ${hiddenStakeholderCount > 0 ? `(${hiddenStakeholderCount})` : ""}`}
          </button>
        )}
      </section>
