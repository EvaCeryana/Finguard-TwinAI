...
function buildRiskMatrix(signals: RiskSignals): RiskMatrixEntry[] {
  ...
  const fallbackByCategory: Record<RiskCategory, RiskMatrixEntry> = {
    ...
    false_positive: genericRisks()[2],
    user_harm: genericRisks()[3],
    operational: genericRisks()[4],
    reputation: genericRisks()[5],
  };

  for (const category of Object.keys(fallbackByCategory) as RiskCategory[]) {
    if (!categories.has(category)) {
      uniqueRisks.push(fallbackByCategory[category]);
    }
  }
...
function buildAbuseScenarios(signals: RiskSignals): AbuseScenario[] {
  ...
  if (signals.scamBlocking) {
    scenarios.push({
      ...
      impact:
        "Some scam payments continue while legitimate users may still be blocked.",
      severity: "high",
    });
  }
...
function buildOperationalRisks(signals: RiskSignals): string[] {
  ...
  if (signals.loanAutomation) {
    risks.push("Appeals and complaints may increase if users do not understand automated lending decisions.");
    risks.push("Risk analytics must monitor approval drift, default rate, and segment-level decision quality.");
  }

  if (signals.scamBlocking) {
    risks.push("Urgent payment escalation process is required for legitimate blocked transactions.");
  }
...
function buildReputationRisks(signals: RiskSignals): string[] {
  ...
  if (signals.transferLimit || signals.withdrawal) {
    risks.push("A high-profile scam or account takeover case could damage confidence in wallet safety.");
  }
  ...
  if (signals.scamBlocking) {
    risks.push("Users may perceive the platform as unreliable if legitimate payments fail without clear explanation.");
  }
